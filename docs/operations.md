...
| `load configuration: permission denied` (repeating, container restart-looping) | `configs/config.yaml` is bind-mounted, so it's read by the container's non-root `mirror` user, not by whoever ran `chmod` on the host. `chown` it to the image's `mirror` UID/GID — see [installation.md](installation.md#docker-recommended). |
| Forgejo webhook delivery fails: `webhook can only call allowed HTTP servers (check your webhook.ALLOWED_HOST_LIST setting)` | Forgejo blocks webhooks to private/internal IPs by default (SSRF protection) — the mirror's Docker-network address is private. Add it to Forgejo's own `app.ini`: `[webhook]` → `ALLOWED_HOST_LIST = external,forgejo-encrypt-mirror` (or the `FORGEJO__webhook__ALLOWED_HOST_LIST` env var), then restart Forgejo. This is a Forgejo-side setting, not `forgejo-encrypt-mirror` config. |
| No webhook delivery recorded at all for an existing repo | An admin-level "Default Webhook" only attaches automatically to repositories created **after** it was added — it doesn't retroactively apply to existing repos. Add the webhook directly on the repo instead (Repo → Settings → Webhooks → Add Webhook). |
| Job fails repeatedly with `git: clone ...: Could not resolve host: ...` even though the hostname resolves fine on the Docker host | The container's DNS resolution is broken even though the host's isn't. Confirm with `docker compose exec <service> nslookup <host> 1.1.1.1` (works) vs. the same command without the explicit server (fails) — if so, add explicit `dns:` servers to the service in your compose file (see [examples/docker-compose.yml](../examples/docker-compose.yml)). |
| Push to GitHub fails with `! [rejected] HEAD -> main (fetch first)` | The destination repo already has a commit that isn't an ancestor of the local encrypted history — usually because it was created manually on GitHub with "Initialize this repository with a README" checked, instead of via `autoCreate` (which creates a genuinely empty repo). Delete the repo on GitHub and let `autoCreate` recreate it empty on the next push, or manually clear its history. |
